...
export default function OrganisationsPage() {
  const { accessToken, profile, refresh, previewReadOnly } = usePlatform();
  ...
  const canCreate = Boolean(profile?.isPlatformAdmin) && !previewReadOnly;
            ...
            <tbody className="divide-y divide-slate-100">
              {data.organisations.map((org) => (
                <tr key={org.id}>
                  ...
                  <Td>
                    <Link href={`/app/foundation-it/organisations/${org.id}`} className="font-medium text-blue-600 hover:text-blue-700">
                      View
                    </Link>
                  </Td>
                </tr>
